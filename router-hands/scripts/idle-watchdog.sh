#!/bin/sh
# Auto-stop router-hands after IDLE_SECONDS of inactivity. Install as a DSM Task Scheduler root
# job on kappa running every 5 minutes. Idle window defaults to 30 minutes.
#
# The relay touches <logs>/last_activity on every tool call; this compares that timestamp to now
# and tears the relay (and its sidecar) down when it goes stale.
...
sudo "$DOCKER" inspect -f '{{.State.Running}}' router-hands 2>/dev/null | grep -q true || exit 0
