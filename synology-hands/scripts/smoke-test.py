...
A bare `curl http://127.0.0.1:8787/mcp` returning 200 does NOT prove auth works, so this runs
an actual MCP initialize + tools/list with a good token (must succeed) and a bad token
(must be rejected with 401).

Run anywhere fastmcp is installed (the Mac, or inside the container via `docker exec`):

    RELAY_URL=http://127.0.0.1:8787/mcp RELAY_TOKEN=<token> python smoke-test.py
...
URL = os.environ.get("RELAY_URL", "http://127.0.0.1:8787/mcp")
